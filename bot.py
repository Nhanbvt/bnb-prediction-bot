...
def place_bet_tx(w3, pred, user_id, address, epoch, direction, amount):
    ...
    try:
        ...
        try:
            estimated = w3.eth.estimate_gas(fn.build_transaction(tx_dict))
...
def do_claim_auto(w3, pred, user_id, address, epochs):
    ...
    try:
        ...
        try:
            estimated = w3.eth.estimate_gas(pred.functions.claim(epochs).build_transaction(tx_dict))
            gas_limit = int(estimated * 1.1)
        except:
            gas_limit = 60000
    ...
    while session["running"]:
        try:
            ...
            for fin in sorted(done_epochs):
                ...
                try:
                    ...
                    if to_claim_list and not session["test_mode"] and wallet:
                        ...
                        ok, tx = do_claim_auto(w3, pred, user_id, wallet["address"], to_claim_list)
                        if ok:
                            to_claim_list = []
                            await context.bot.send_message(chat_id, f"💰 Claim thành công!")
            ...
            if is_new_round and in_bet_window and epoch not in bet_epochs_done:
                ...
                msg = f"🔮 *ROUND #{epoch}*\n🎯 {pred_emoji} | 💸 {BET_AMOUNT} BNB"
                await context.bot.send_message(chat_id, msg, parse_mode="Markdown", reply_markup=stop_kb())
                ...
                if session["test_mode"]:
                    ...
                    if wallet:
                        ok, tx = place_bet_tx(w3, pred, user_id, wallet["address"], epoch, direction, BET_AMOUNT)
                        if ok:
                            pending_bets[epoch] = {"pred": direction, "amount": BET_AMOUNT}
                            bet_epochs_done.add(epoch)
                            await context.bot.send_message(
                                chat_id, f"✅ Đặt {BET_AMOUNT} → {pred_emoji}", parse_mode="Markdown"
                            )
                        else:
                            await context.bot.send_message(chat_id, f"❌ {tx}")
