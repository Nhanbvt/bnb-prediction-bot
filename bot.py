...
def do_claim_auto(w3, pred, user_id, address, epochs):
    """CLAIM TỰ ĐỘNG - GAS CỐ ĐỊNH TỐI ƯU"""
    try:
        from web3 import Web3
    ...
    async def check_results():
        while session["running"]:
            try:
                ...
                logger.error(f"Check results: {e}")
                await asyncio.sleep(5)

    # TASK 2: Đặt cược mới (song song)
    async def place_bets():
        ...
        while session["running"]:
            try:
                ...
                # ĐẶT CƯỢC KHI ROUND MỚI BẮT ĐẦU
