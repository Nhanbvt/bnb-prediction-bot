...
def init_wallet(user_id, address, private_key):
    ...
    user_wallets[user_id] = {"address": address, "encrypted_key": encrypted}
...
def claim_tx(w3, pred, user_id, address, epochs):
    try:
        from web3 import Web3
        pk = get_private_key(user_id)
        addr = Web3.to_checksum_address(address)
...
def check_stop(session):
    ...
    if session["max_rounds"] > 0 and total >= session["max_rounds"]:
        return f"✅ Hoàn thành {session['max_rounds']} rounds!"
    return None

async def send_final_report(context, chat_id, session):
    total = session["total_rounds"]
    ...
    try:
        ...
        # ── TEST MODE ──
            ...
            session.update({
                ...
                "running": True, "consecutive_losses": 0,
                "current_streak": 0, "max_consecutive_wins": 0,
                "max_consecutive_losses": 0,
            })
            await query.edit_message_text(
                f"🧪 *TEST BẮT ĐẦU!*\n━━━━━━━━━━━━━━━━━━\n\n"
    ...
    if waiting == "seed":
        ...
        if len(text.strip().split()) != 12:
            await context.bot.send_message(update.effective_chat.id,
                "❌ Phải đúng 12 từ! Thử lại /start → Cài ví.")
            return
...
async def bot_loop(context, user_id: int, chat_id: int):
    ...
    # Track các round đang chờ kết quả
    ...
    while session["running"]:
        try:
            ...
            # ── CLAIM / KẾT QUẢ CÁC ROUND ĐÃ QUA ──
            finished_epochs = [e for e in list(pending_bets.keys()) if e < epoch]
            for fin_epoch in sorted(finished_epochs):
                await asyncio.sleep(8)
                try:
                    rd = pred.functions.rounds(fin_epoch).call()
                    if not rd[13]:  # oracle chưa cập nhật
                        await asyncio.sleep(10)
                        rd = pred.functions.rounds(fin_epoch).call()

                    bet_info  = pending_bets[fin_epoch]
                    ...
                    if won:
                        ...
                        result_text = f"❌ THUA -{loss:.4f} BNB"

                    session["gas_used"]     = round(session["gas_used"] + gas, 6)
                    ...
                    await context.bot.send_message(
                        chat_id=chat_id,
                        text=(
                            f"{'✅' if won else '❌'} *ROUND #{fin_epoch}*\n"
                            f"━━━━━━━━━━━━━━━━━━\n\n"
                            f"🎯 Đặt: {bet_info['pred']} | Kết quả: {actual}\n"
                            f"💰 Cược: {bet_amt:.4f} BNB\n"
                            f"⛽ Gas: {gas:.4f} BNB\n"
                            f"{result_text}\n\n"
                            f"━━ 📊 TỔNG KẾT ━━\n"
                            f"💼 Số dư: {session['balance']:.4f} BNB\n"
                    ...
                    if stop_reason:
                        session["running"] = False
                        await context.bot.send_message(
                            chat_id, f"🏁 Lý do dừng: {stop_reason}")
                        await send_final_report(context, chat_id, session)
                        return

                except Exception as e:
                    logger.error(f"Result error epoch {fin_epoch}: {e}")
                    if fin_epoch in pending_bets:
                        del pending_bets[fin_epoch]

            # ── PHÂN TÍCH & ĐẶT CƯỢC ──
            time_to_lock = lock_time - int(time.time())

            # Lần đầu: đặt 2 round liên tiếp (epoch hiện tại + epoch+1)
            if is_first_start:
                epochs_to_bet = []
                if epoch not in last_bet_epochs and time_to_lock > 10:
                    epochs_to_bet.append(epoch)
                next_epoch = epoch + 1
                if next_epoch not in last_bet_epochs:
                    epochs_to_bet.append(next_epoch)

                for bet_epoch in epochs_to_bet:
                    ...
                    ep_pool   = get_pool_info(pred, bet_epoch)
                    pool_text = ""
                    if ep_pool:
                        pool_text = (
                            f"🐂 Bull: {ep_pool['bull_pct']}% (x{ep_pool['bull_payout']})\n"
                            f"🐻 Bear: {ep_pool['bear_pct']}% (x{ep_pool['bear_payout']})"
                        )

                    btc_text = (f"₿ BTC: {btc['change']:+.2f}% "
                                f"{'⬆️' if btc['trend']=='UP' else '⬇️'}"
                                ) if btc else "₿ BTC: N/A"

                    # Tính cược - tối thiểu 0.001
                    bet_amount = max(round(session["balance"] * 0.05, 6), 0.001)
                    ...
                    await context.bot.send_message(
                        chat_id=chat_id,
                        text=(
                            f"🔮 *PHÂN TÍCH ROUND #{bet_epoch}* {mode_txt} 🚀 KHỞI ĐỘNG\n"
                            f"━━━━━━━━━━━━━━━━━━\n\n"
                            ...
                            f"Cược: *{bet_amount:.4f} BNB* (5% của {session['balance']:.4f})"
                        ),
                        parse_mode="Markdown",
                        reply_markup=stop_kb()
                    )
                    ...
                    if session["test_mode"]:
                        pending_bets[bet_epoch] = {"pred": direction, "amount": bet_amount}
                        last_bet_epochs.append(bet_epoch)
                    else:
                        if wallet:
                            ok, tx = place_bet_tx(
                                w3, pred, user_id,
                                wallet["address"], bet_epoch, direction, bet_amount
                            )
                            if ok:
                                pending_bets[bet_epoch] = {"pred": direction, "amount": bet_amount}
                                last_bet_epochs.append(bet_epoch)
                                await context.bot.send_message(
                                    chat_id,
                                    f"✅ Đặt round #{bet_epoch}: {bet_amount:.4f} BNB → {pred_emoji}\n"
                                    f"`{tx[:20]}...`",
                                    parse_mode="Markdown"
                                )
                            else:
                                await context.bot.send_message(
                                    chat_id, f"❌ Đặt cược thất bại round #{bet_epoch}:\n{tx}")

                is_first_start = False

            else:
                # Từ round 3 trở đi: chỉ đặt 1 round tiếp theo
                next_epoch   = epoch + 1
                time_to_lock = lock_time - int(time.time())

                if next_epoch not in last_bet_epochs and time_to_lock > 5:
                    if time_to_lock > 25:
                    ...
                    next_pool  = get_pool_info(pred, next_epoch)
                    pool_text  = ""
                    if next_pool:
                        pool_text = (
                    ...
                    # Tính cược 5% số dư hiện tại - tối thiểu 0.001
                    bet_amount = max(round(session["balance"] * 0.05, 6), 0.001)
                    if not session["test_mode"]:
                        bet_amount = max(round(bet_amount - 0.0005, 6), 0.001)
                    ...
                    await context.bot.send_message(
                        chat_id=chat_id,
                        text=(
                            f"🔮 *PHÂN TÍCH ROUND #{next_epoch}* {mode_txt}\n"
                            f"━━━━━━━━━━━━━━━━━━\n\n"
                            f"💵 BNB: ${analysis['bnb_price']} | CL: ${analysis['cl_price']}\n"
                            f"📊 Chênh: {analysis['price_diff']}%\n\n"
                            f"━━ 1M ━━\n"
                    ...
                    if session["test_mode"]:
                        pending_bets[next_epoch] = {"pred": direction, "amount": bet_amount}
                        last_bet_epochs.append(next_epoch)
                    else:
                        if wallet:
                            ok, tx = place_bet_tx(
                                w3, pred, user_id,
                                wallet["address"], next_epoch, direction, bet_amount
                            )
                            if ok:
                                pending_bets[next_epoch] = {"pred": direction, "amount": bet_amount}
                                last_bet_epochs.append(next_epoch)
                                await context.bot.send_message(
                                    chat_id,
                                    f"✅ Đặt {bet_amount:.4f} BNB → {pred_emoji}\n"
                                    f"`{tx[:20]}...`",
                                    parse_mode="Markdown"
                                )
                            else:
