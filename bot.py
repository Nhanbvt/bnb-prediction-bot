...
def get_session(user_id):
    if user_id not in bot_sessions:
        bot_sessions[user_id] = {
            ...
            "gas_used": 0.0,
            "waiting_for": None,
            # Chiến lược chọn hướng:
            # "ai"       = phân tích indicator (mặc định cũ)
            # "random"   = random 50/50 hoàn toàn
            # "pool"     = chọn bên ít tiền hơn (payout cao hơn)
            # "anti_ai"  = ngược lại signal AI
            "strategy": "ai",
        }
...
def pick_direction(strategy, analysis, pool_info):
    """
    Chọn hướng đặt cược theo chiến lược.
    - ai       : theo signal indicator
    - random   : random 50/50
    - pool     : chọn bên ít tiền hơn (payout cao hơn)
    - anti_ai  : ngược signal AI
    """
    if strategy == "random":
        return random.choice(["UP", "DOWN"])

    if strategy == "pool" and pool_info:
        # Chọn bên có payout cao hơn (ít người đặt hơn)
        if pool_info["bull_payout"] >= pool_info["bear_payout"]:
            return "UP"
        else:
            return "DOWN"
...
async def send_main(target, user_id, edit=False):
    ...
    if wallet:
        try:
            ...
            wallet_text = (f"✅ <code>{wallet['address'][:8]}...{wallet['address'][-6:]}</code>\n"
                           f"💰 Số dư: {bal} BNB")
        except:
            wallet_text = f"✅ <code>{wallet['address'][:8]}...{wallet['address'][-6:]}</code>"
    text = f"🤖 <b>BNB PREDICTION BOT</b>\n━━━━━━━━━━━━━━━━━━\n\n{wallet_text}"
    if edit:
        await target.edit_message_text(text, parse_mode="HTML", reply_markup=main_kb())
    else:
